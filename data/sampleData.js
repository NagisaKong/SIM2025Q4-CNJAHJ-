// In-memory data store implemented as an object-oriented service so that
// future persistence layers (databases, APIs, etc.) can replace this class
// with minimal changes to the application code. The class encapsulates all
// CRUD-style helpers that power the Express middleware.
class VolunteerPlatformDataStore {
  ...
  // Getter helpers provide read-only access to the collections used by the views.
  getRequestStatuses() {
  ...
  getPinRequests() {
    return this.pinRequests;
  }

  getCsrShortlist() {
    return this.csrShortlist;
  }

  getCsrHistory() {
    return this.csrHistory;
  }
  ...
  // Account and profile management methods
  ...
  searchUserAccounts(keyword) {
    if (!keyword) return this.userAccounts;
  ...
  searchUserProfiles(keyword) {
    if (!keyword) return this.userProfiles;
    const lower = keyword.toLowerCase();
